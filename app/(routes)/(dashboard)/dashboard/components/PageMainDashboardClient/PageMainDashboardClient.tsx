...
export function PageDashboardClient({ strategies }: PageMainDashboardProps) {
  ...
  useEffect(() => {
    ...
    if (filters.description) {
      filtered = filtered.filter((strategy) => 
        strategy.description.toLowerCase().includes(filters.description.toLowerCase()
      ))
    }
  ...
  return (
    <div>
      ...
      <FiltersStrategies setFilters={handleFilterChange} clearFilters={clearFilters} filters={filters}/>
      <ListStrategies strategies={filteredStrategies} />
    </div>
  )
}